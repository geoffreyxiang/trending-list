        ...
        Item.findOne({ name: req.body.name, restaurant_name: req.body.restaurant_name }).then( async (currentItem) => {
            ...
            if (currentItem === null) {
                ...
                const item = new Item({
                    ...
                    order_history: [{ date: Date.now(), quantity: req.body.quantity }],
            ...
            // if the item exists, we must update the order_history array accordingly
                ...
                new_order_history.push({ date: Date.now(), quantity: req.body.quantity })
...
const placeOrder = async (req, res) => {
    try {
        ...
        for (const item of order) {
            const orderedItem = await axios.put('http://localhost:3030/orders/item', {name: item.name, restaurant_name: restaurant_name, price: item.price, quantity: item.quantity})
            .then((res) => res.data)
...
module.exports = { orderItem, placeOrder }